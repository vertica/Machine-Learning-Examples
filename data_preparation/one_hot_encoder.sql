...
SELECT GET_MODEL_SUMMARY(USING PARAMETERS model_name='public.titanic_encoder');

SELECT * FROM (SELECT GET_MODEL_ATTRIBUTE(USING PARAMETERS model_name='public.titanic_encoder', attr_name='varchar_categories')) AS attrs
...
CREATE VIEW titanic_training_encoded AS SELECT passenger_id, survived, pclass, sex_1, age, sibling_and_spouse_count, parent_and_child_count, fare, embarkation_point_1, embarkation_point_2 FROM
    (SELECT APPLY_ONE_HOT_ENCODER(* USING PARAMETERS model_name='public.titanic_encoder') FROM titanic_training) AS sq;
...
SELECT passenger_id, name,
    PREDICT_LOGISTIC_REG(pclass, sex_1, age, sibling_and_spouse_count, parent_and_child_count, fare,
                         embarkation_point_1, embarkation_point_2 USING PARAMETERS model_name='public.titanic_log_reg')
